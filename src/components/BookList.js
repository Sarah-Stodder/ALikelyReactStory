...
//would like to make it where this will change based to user books once they hit a button.
    ...
    const handleInfo= (book) =>{
        console.log("I would want it to go to my show book page", book.title)
    ...
    <ImageList cols={3} >
        {books.map((book)=>(
            <ImageListItem key={book.id}>
                <img sx={{height:"200px"}} src={book.img} srcSet={book.img} alt={book.name}  loading="lazy"/>
                <ImageListItemBar
                title={book.title}
                ...
                actionIcon={<>
                    <IconButton sx={{ color: 'rgba(255, 255, 255, 0.54)' }} aria-label={`info about ${book.title}`} onClick={()=>handleInfo(book)}>
                        <InfoIcon/>
                    </IconButton>
